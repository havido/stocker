...
import { MOCK_PRICES, type PricePoint, type StockData } from "@/lib/mockData";
...
export function StockChart({ stock, loading }: StockChartProps) {
  ...
  const data: PricePoint[] = MOCK_PRICES[period] || [];
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6">
      <div className="mb-1 flex items-baseline gap-3">
        ...
        <span className="text-sm text-muted-foreground font-medium">{stock.name}</span>
      </div>
      ...
      <div className="h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            <XAxis dataKey="time" tick={{ fontSize: 11 }} stroke="hsl(var(--muted-foreground))" tickLine={false} axisLine={false} interval="preserveStartEnd" />
            <YAxis domain={["auto", "auto"]} tick={{ fontSize: 11 }} stroke="hsl(var(--muted-foreground))" tickLine={false} axisLine={false} width={50} />
            <Tooltip
              contentStyle={{
